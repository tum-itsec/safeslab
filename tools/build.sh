...
build_rootfs()
{
    ...
    sudo debootstrap --help > /dev/null || exit 1    
    ...
    # setup key to ssh into guest
    sudo mkdir -p ${MOUNT_DIR}/${VM_ROOT_DIR}/.ssh
    cat ${TOOL_DIR}/rootfs-id_ed25519.pub | sudo tee ${MOUNT_DIR}/${VM_ROOT_DIR}/.ssh/authorized_keys > /dev/null
